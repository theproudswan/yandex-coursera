...
void PrintStats(vector<Person> persons){
    ...
    cout << "Median age for employed females = "<<ComputeMedianAge(persons.begin(), 
                                            partition(persons.begin(), persons.end(), 
                                                [](const Person& person){
                                                    return (person.gender == Gender::FEMALE)&&(person.is_employed == 1);
                                            }))<<endl;
    cout << "Median age for unemployed females = "<<ComputeMedianAge(persons.begin(), 
                                            partition(persons.begin(), persons.end(), 
                                                [](const Person& person){
                                                    return (person.gender == Gender::FEMALE)&&(person.is_employed == 0);
                                            }))<<endl;
    cout << "Median age for employed males = "<<ComputeMedianAge(persons.begin(), 
                                            partition(persons.begin(), persons.end(), 
                                                [](const Person& person){
                                                    return (person.gender == Gender::MALE)&&(person.is_employed == 1);
                                            }))<<endl;
    cout << "Median age for unemployed males = "<<ComputeMedianAge(persons.begin(), 
                                            partition(persons.begin(), persons.end(), 
                                                [](const Person& person){
                                                    return (person.gender == Gender::MALE)&&(person.is_employed == 0);
                                            }))<<endl;
}
