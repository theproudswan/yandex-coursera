...
public:
  ...
  struct Bucket{
	  MapType bucket_data;
	  mutable mutex bucket_data_mutex;

	  bool Has(const K& key) const {
		  return bucket_data.count(key) > 0;
	  }

	  WriteAccess GetValue(const K& key) {
		  return {lock_guard(bucket_data_mutex), bucket_data[key]};
	  }
	  ReadAccess GetValue(const K& key) const {
		  return {lock_guard(bucket_data_mutex), bucket_data.at(key)};
	  }
  };
  ...
  MapType BuildOrdinaryMap() const {
	    ...
	    for (const auto& bucket : data){
	    	lock_guard<mutex> lock(bucket.bucket_data_mutex);
	    	for(const auto& [k, v] : bucket.bucket_data){
	    		result[k] = v;
	    	}
	    }
...
private:
  ...
  Bucket& GetBucket(const K& key){
	  return data[hasher(key) % data.size()];
  }
