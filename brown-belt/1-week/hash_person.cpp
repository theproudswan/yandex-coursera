...
struct AddressHasher {
  ...
  size_t operator () (const Address& address) const {
      return (773*773*shash(address.city) + 773*shash(address.street) + ihash(address.building));
  }
  hash<string> shash;
  hash<int> ihash;
};
...
struct PersonHasher {
  ...
  size_t operator () (const Person& person) const {
      return 997*997*997*shash(person.name) + 
             997*997*ihash(person.height) +
             997*dhash(person.weight) +
